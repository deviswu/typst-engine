...
| A2 | 导出不重算 | 调用计数器 | 同 revision 计算次数 = 1 | ⏳ Plan 2（`export/` 未建） |
...
| A6 | 失败不白屏 | 故意引入语法错误 | 预览仍有内容 + 波浪线出现 | 🟡 错误不 panic + 2 条诊断已验；`success_doc` 在 Plan 2 |
...
> 以上是**单次运行**的样本，不是 P95。要让 A3/A4 成为稳定门槛，需在 Plan 2 里跑热身后的多次统计。
> 目前这些数字足以证明**机制成立**。
>
> A2 / A5 仍是空白，不是失败 —— 对应代码（`export/`、`syntax-svc`）还没写。
