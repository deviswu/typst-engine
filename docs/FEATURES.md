...
| `dirty` 用**事实**算，不是「变过就一直脏」 | 编辑器每次文本变化都重算 `dirty = 文本 != 磁盘上那份`。旧写法只在保存 / 加载时清 `dirty`，于是「改了又撤销回原状」会让它**永远卡在 true**：自动保存判定「没变化」不写、点文件一直被拒（实测日志里就是这个形状：自动保存停在第 165 次之后，全是「拒绝打开」） | `main.rs::on_editor_change` |
...
| 外部改动时不重读的情况 | ① **本地有未保存改动**：两边都留着，状态栏一句「磁盘上被改过 —— 编辑器里有未保存的改动，先两边都留着（保存或撤到原状后会自动加载；点目录树里这个文件 = 立刻丢掉本地改动加载）」② 内置示例文档（它不是用户的文件）。文件被删 / 读不动也一样只提示 | `disk_watch::verdict`（纯函数，三态有单测）· `main.rs::note_disk_conflict` |
| **待装的磁盘新版不会丢** | 「本地脏 → 先不覆盖」那一刻的改动记在 `disk_conflict` 里（旧写法只把指纹记下了事，等于把这次改动**消费掉**：等你保存完也不会再装）。本地一变干净（保存了 / 撤回原状了），轮询里的 ⓪ 步就把它补装上 | `main.rs::spawn_disk_watch` ⓪ · `note_disk_conflict` |
| **写盘前不静默覆盖**（防丢数据） | `Ctrl+S` 与自动保存共用的 `write_to_disk` 先比指纹：磁盘上那份被外部改过就**不写**，状态栏给两个出口：`Ctrl+S` **再按一次** = 以我为准写盘；或点目录树里这个文件 = 丢掉本地改动、加载磁盘那份。**自动保存遇到冲突一律不写**（「谁赢」由用户决定，不让一秒后的定时器替他决定） | `disk_watch::may_overwrite`（纯函数，含单测）· `main.rs::{write_to_disk,save_file,autosave_now}` |
| **点目录树里的文件 = 从磁盘重新读**（脏时两下确认） | 本地干净：点一下就直接从磁盘重载（包括点的就是当前那个文件 —— 这就是「在终端里改完、回 app 点一下看到」的入口）。本地有未保存改动：**第一下只提示、第二下（8 秒内）才真丢改动重载**（实测用户被无提示拒绝 23 次，那一下不能是死胡同） | `main.rs::{open_path,pending_open}` · `disk_watch::within_confirm_window`（含单测） |
| 这一整套的端到端自测 | `cargo run -p typst-live -- --watch-selftest tmp/x.typ` —— 走真实代码路径（不注入键盘鼠标）：弄脏 → 外部改写 → 断言「本地改动保住 / 磁盘上那份没被自动保存盖掉 / 第一下点只提示 / 第二下点加载了磁盘新版」。只接受 `tmp/` 或临时目录下的文件（它会真改那个文件） | `main.rs::{WATCH_SELFTEST,watch_selftest_step}` |
