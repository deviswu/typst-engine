...
编辑区上方还有一排**工具栏**（11 个按钮）：标题 / 粗 / 斜 / 上标 / 下标 /
公式 / 对齐 / 颜色 / 图片 / 表格 / 分页。有选区就包住选区，没有就插一对空壳
并把光标夹在中间（接着打就行）。逻辑在 `crates/app/src/markup.rs`，纯函数 + 单测 ——
中文选区按字节算不切半个字、标题插**行首**而不是光标处，这两条都被测到了。
